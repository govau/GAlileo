...
with models.DAG(
    ...
    query_device_category_snapshot = bigquery_operator.BigQueryOperator(
        ...
        bql=pathlib.Path(galileo.DAGS_DIR+"/bq_scripts/dta_sql_devicecategory_daily_snapshot_full_emp").read_text(), use_legacy_sql=False)
    ...
    query_device_browser_snapshot = bigquery_operator.BigQueryOperator(
        ...
        bql=pathlib.Path(galileo.DAGS_DIR+"/bq_scripts/dta_sql_devicebrowser_daily_snapshot_full_emp").read_text(), use_legacy_sql=False)
    ...
    query_device_ops_snapshot = bigquery_operator.BigQueryOperator(
        ...
        bql=pathlib.Path(galileo.DAGS_DIR+"/bq_scripts/dta_sql_deviceops_daily_snapshot_full_emp").read_text(), use_legacy_sql=False)
    ...
    query_traffic_src_medium_snapshot = bigquery_operator.BigQueryOperator(
        ...
        bql=pathlib.Path(galileo.DAGS_DIR+"/bq_scripts/dta_sql_traffic_sourcemedium_daily_snapshot_full_emp").read_text(), use_legacy_sql=False)
    ...
    query_traffic_src_medium_delta_snapshot = bigquery_operator.BigQueryOperator(
        ...
        bql=pathlib.Path(galileo.DAGS_DIR + "/bq_scripts/dta_sql_traffic_sourcemedium_daily_snapshot_delta_emp").read_text(), use_legacy_sql=False)
    ...
    query_geolocation_snapshot = bigquery_operator.BigQueryOperator(
        ...
        bql=pathlib.Path(galileo.DAGS_DIR+"/bq_scripts/dta_sql_geolocation_daily_snapshot_full_emp").read_text(), use_legacy_sql=False)

    query_local_city_delta_snapshot = bigquery_operator.BigQueryOperator(
        task_id='query_local_city_delta_snapshot',
        bql=pathlib.Path(galileo.DAGS_DIR + "/bq_scripts/dta_sql_geolocation_localcity_daily_snapshot_delta_emp").read_text(), use_legacy_sql=False)

    query_country_delta_snapshot = bigquery_operator.BigQueryOperator(
        task_id='query_country_delta_snapshot',
        bql=pathlib.Path(galileo.DAGS_DIR + "/bq_scripts/dta_sql_geolocation_country_daily_snapshot_delta_emp").read_text(), use_legacy_sql=False)
    ...
    query_user_session_snapshot = bigquery_operator.BigQueryOperator(
        ...
        bql=pathlib.Path(galileo.DAGS_DIR + "/bq_scripts/dta_sql_sessions_user_daily_snapshot_full_emp").read_text(), use_legacy_sql=False)
    
    query_user_session_delta_snapshot = bigquery_operator.BigQueryOperator(
        task_id='query_user_session_delta_snapshot',
        bql=pathlib.Path(galileo.DAGS_DIR + "/bq_scripts/dta_sql_sessions_user_daily_snapshot_delta_emp").read_text(), use_legacy_sql=False)
    ...
    query_device_opsbrowser_snapshot = bigquery_operator.BigQueryOperator(
        ...
        bql=pathlib.Path(galileo.DAGS_DIR+"/bq_scripts/dta_sql_device_opsbrowser_daily_snapshot_full_emp").read_text(), use_legacy_sql=False)

    query_device_opsbrowser_delta_snapshot = bigquery_operator.BigQueryOperator(
        task_id='query_device_opsbrowser_delta_snapshot',
        bql=pathlib.Path(galileo.DAGS_DIR + "/bq_scripts/dta_sql_device_opsbrowser_daily_snapshot_delta_emp").read_text(), use_legacy_sql=False)
    # ============================================================================================================
    # Export datasets
    # pageviews snapshot
    ...
    export_bq_to_gcs_json_traffic_src_medium = bigquery_to_gcs.BigQueryToCloudStorageOperator(
        ...
        source_project_dataset_table="{{params.project_id}}.dta_customers.pageviews_daily_snapshot_traffic_src_medium_delta_emp",
        params={
            'project_id': project_id
        },
        destination_cloud_storage_uris=[
            "gs://%s/data/analytics/json/%s.json" % (
                models.Variable.get('AIRFLOW_BUCKET',
                                    'us-east1-dta-airflow-b3415db4-bucket'),
                'traffic_src_medium_daily_snapshot_emp')],
        export_format='NEWLINE_DELIMITED_JSON')
    ...
    export_bq_to_gcs_json_local_city = bigquery_to_gcs.BigQueryToCloudStorageOperator(
        ...
        source_project_dataset_table="{{params.project_id}}.dta_customers.pageviews_daily_snapshot_local_city_delta_emp",
        params={
            'project_id': project_id
        },
        destination_cloud_storage_uris=[
    ...
    export_bq_to_gcs_json_country = bigquery_to_gcs.BigQueryToCloudStorageOperator(
        ...
        source_project_dataset_table="{{params.project_id}}.dta_customers.pageviews_daily_snapshot_country_delta_emp",
        params={
            'project_id': project_id
        },
        destination_cloud_storage_uris=[
    ...
    export_bq_to_gcs_json_session_users = bigquery_to_gcs.BigQueryToCloudStorageOperator(
        ...
        source_project_dataset_table="{{params.project_id}}.dta_customers.pageviews_daily_snapshot_session_user_delta_emp",
        params={
            'project_id': project_id
        },
        destination_cloud_storage_uris=[
            "gs://%s/data/analytics/json/%s.json" % (
                models.Variable.get('AIRFLOW_BUCKET',
                                    'us-east1-dta-airflow-b3415db4-bucket'),
                'session_users_daily_snapshot_emp')],
        export_format='NEWLINE_DELIMITED_JSON')
    ...
    export_bq_to_gcs_json_device_opsbrowser = bigquery_to_gcs.BigQueryToCloudStorageOperator(
        ...
        source_project_dataset_table="{{params.project_id}}.dta_customers.pageviews_daily_snapshot_device_opsbrowser_delta_emp",
        params={
            'project_id': project_id
        },
        destination_cloud_storage_uris=[
            "gs://%s/data/analytics/json/%s.json" % (
                models.Variable.get('AIRFLOW_BUCKET',
                                    'us-east1-dta-airflow-b3415db4-bucket'),
                'device_opsbrowser_daily_snapshot_emp')],
        export_format='NEWLINE_DELIMITED_JSON')
