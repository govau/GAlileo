...
def combine_tally():
    ...
    with open(DATA_DIR+'tally_69211100_20190425.csv', 'wb') as f:
        f.write('path,hits\n')
        f.write(data.csv)
...
def generate_chart():
    ...
    df = Dataset().load(open(DATA_DIR+'tally_69211100_20190425.csv').read()).df.sort_values(by=['hits'])
    df = df[df['hits'] > 30]

    p = (ggplot(df, aes(x='path', y='hits')) + geom_col())
    p.save('test.png')
...
with models.DAG(
    ...
    benchmark_tally >> combine_tally >> tell_slack
